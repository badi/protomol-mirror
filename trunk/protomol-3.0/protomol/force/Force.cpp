...
namespace ProtoMol {
  ...
  CompareForce *Force::makeCompareForce(Force *actualForce,
    ...
    (actualForce ? actualForce->getId() : string("null pointer")) << " vs. " <<
    (compareForce ? compareForce->getId() : string("null pointer")) << "." <<
    endr;
    return NULL;
  }
  ...
  TimeForce *Force::makeTimeForce(Force *actualForce) const {
    ...
    (actualForce ? actualForce->getId() : string("null pointer")) << "." <<
    endr;
    return NULL;
  }
