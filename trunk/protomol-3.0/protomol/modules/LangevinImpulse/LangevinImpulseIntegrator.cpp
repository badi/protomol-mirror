...
LangevinImpulseIntegrator::LangevinImpulseIntegrator(
  Real timestep, Real LangevinTemperature, Real gamma, int seed,
  ForceGroup *overloadedForces) :
  STSIntegrator(timestep,
                overloadedForces), myLangevinTemperature(LangevinTemperature),
...
void LangevinImpulseIntegrator::fluctuationLI() {
  ...
  const Real forceConstant = 2 * Constant::BOLTZMANN *
                             myLangevinTemperature * myGamma;
  ...
  for (unsigned int i = 0; i < myPositions->size(); i++) {
    ...
    Vector3D gaussRandCoord1(randomGaussianNumber(
                               mySeed), randomGaussianNumber(
                               mySeed), randomGaussianNumber(mySeed));
    Vector3D gaussRandCoord2(randomGaussianNumber(
                               mySeed), randomGaussianNumber(
                               mySeed), randomGaussianNumber(mySeed));
    ...
    (*myVelocities)[i] = (*myVelocities)[i] *
                         exp(-myGamma *
                             dt) + gaussRandCoord1 * sqrtFCoverM * sqrtTau2;
  }
}
...
const {
  ...
  parameters.push_back(Parameter("temperature",
                                 Value(myLangevinTemperature,
                                       ConstraintValueType::NotNegative())));
  parameters.push_back(Parameter("gamma",
                                 Value(myGamma *
                                       (1000 * Constant::INV_TIMEFACTOR),
                                       ConstraintValueType::NotNegative())));
  parameters.push_back(Parameter("seed",
                                 Value(mySeed,
                                       ConstraintValueType::NotNegative()),
                                 1234));
}
