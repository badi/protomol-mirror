...
namespace ProtoMol {
  ...
  class NonbondedCutoffForce : public TForce,
    private NonbondedCutoffForceBase {
    //~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
    // Typedef
    //~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
  private:
  ...
  const {
    return
      new TImplForce(values[values.size() - 1],
                     TOneAtomPair::make(std::vector<Value>(values.begin(),
                                                           values.end() - 1)));
  }

  template<class TCellManager, class TOneAtomPair, class TForce,
           class TImplForce>
  std::string NonbondedCutoffForce<TCellManager, TOneAtomPair, TForce,
                                   TImplForce>::getIdNoAlias() const {
