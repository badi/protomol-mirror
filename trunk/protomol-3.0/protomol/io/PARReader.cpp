...
namespace ProtoMol {
  ...
  PARReader::PARReader(const std::string &filename,
  ...
  bool PARReader::open(const std::string &filename,
                       PAR::CharmmTypeEnum charmmType) {
  ...
  bool PARReader::read(PAR &par) {
    ...
    for (vector<vector<string> >::const_iterator i = input.begin();
      ...
      case NONBONDED: {
        ...
        nonbonded.polarizability2 = (s.size() >= 7 ? toReal(j[4]) : 0.0);
        nonbonded.epsilon14 =
          (s.size() >=
          5 ? toReal(j[3 + (isCharmm19 ? 0 : 2)]) : nonbonded.epsilon);
        nonbonded.sigma14 =
          (s.size() >=
          5 ? toReal(j[4 +
                       (isCharmm19 ? 0 : 2)]) : nonbonded.sigma) *
          (isCharmm19 ? PAR::Nonbonded::SIGMA_CHARMM19_TO_CHARMM28 : 1.0);
