...
namespace ProtoMol {
  ...
  public:
    ...
    SemiGenericTopology(Real csf,
                        const ExclusionType &e,
                        const TBoundaryConditions &b) : GenericTopology(csf,
                                                                        e),
  ...
  public:
    ...
    virtual Vector3D minimalDifference(const Vector3D &c1,
                                       const Vector3D &c2) const {return
                                                                         boundaryConditions
                                                                           .minimalDifference(c1,
                                                                    c2);}
    /// Find the position in the basis/original cell/image.
    virtual Vector3D minimalPosition(const Vector3D &c) const {return Vector3D(
                                                                 boundaryConditions
                                                                   .
                                                                   minimalPosition(c));}
    ...
    virtual void getBoundingbox(const Vector3DBlock &positions,
      ...
      if (positions.size() <= 0) {
        minbb = Vector3D(Constant::MAXREAL,
          Constant::MAXREAL,
          Constant::MAXREAL);
      ...
      for (unsigned int i = 1; i < count; ++i) {
        ...
        if (pos.x < minbb.x)
          minbb.x = pos.x;
        else if (pos.x > maxbb.x)
          maxbb.x = pos.x;
        if (pos.y < minbb.y)
          minbb.y = pos.y;
        else if (pos.y > maxbb.y)
          maxbb.y = pos.y;
        if (pos.z < minbb.z)
          minbb.z = pos.z;
        else if (pos.z > maxbb.z)
          maxbb.z = pos.z;
      }
    }
    ...
    const {
      for (unsigned int i = 0; i < this->molecules.size(); ++i)
        for (unsigned int j = 0; j < this->molecules[i].pairs.size(); ++j) {
          ...
          if (boundaryConditions.minimalTranslationDifference(c1,
                c2).normSquared() > Constant::EPSILON)
            return false;
        }
