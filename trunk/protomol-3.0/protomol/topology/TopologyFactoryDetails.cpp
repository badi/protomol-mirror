...
namespace ProtoMol {
  //________________________________________ TopologyFactoryDetails
  TopologyFactoryDetails::TopologyFactoryDetails() :
    FactoryBase<GenericTopology>() {
  }
  ...
  TopologyFactoryDetails &TopologyFactoryDetails::operator=(
    const TopologyFactoryDetails &) {
    return *this;
  }

  void TopologyFactoryDetails::registerAllExemplarsConfiguration(
    Configuration *config) {
    TFactory::instance().doRegisterAllExemplarsConfiguration(config);
  }

  GenericTopology *TopologyFactoryDetails::make(string &errMsg,
  ...
  GenericTopology *TopologyFactoryDetails::doMake(string &errMsg,
                                                  const string &id,
                                                  const vector<Value> &values)
  const {
    errMsg = "";
  ...
  void TopologyFactoryDetails::doRegisterAllExemplarsConfiguration(
    Configuration *config) const {
    for (set<const GenericTopology *>::const_iterator i = myPointers.begin();
         i != myPointers.end();
         ++i) {
  ...
  string TopologyFactoryDetails::doPrint() const {
    ...
    for (map<string, const GenericTopology *,
             ltstrNocase>::const_iterator i = myExemplars.begin();
         i != myExemplars.end();
         ++i) {
    ...
    res += "\nAlias:";
    for (map<string, const GenericTopology *,
             ltstrNocase>::const_iterator j = myAliasExemplars.begin();
         j != myAliasExemplars.end();
         ++j)
