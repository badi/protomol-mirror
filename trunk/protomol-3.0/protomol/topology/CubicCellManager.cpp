...
namespace ProtoMol {
  ...
  CubicCellManager::CubicCellManager(Real r) : myCellSize(r),
    myRealCellSize(Vector3D(r, r,
                            r)),
  ...
  void CubicCellManager::getParameters(vector<Parameter> &parameters) const {
    parameters.push_back(Parameter("cellSize",
        Value(myCellSize,
          ConstraintValueType::Positive()),
        Text(
          "For Periodic BC this must be < least cell basis vector.  Typically 1/2 of the least cutoff value")));
  }

  CubicCellManager CubicCellManager::make(string &errMsg,
                                          vector<Value> values) {
    Real r = 1.0;
    if (!values[0].get(r) || r <= 0.0) {
      errMsg += keyword + " cellmanager: cutoff > 0 (" +
                values[0].getString() + ").";
      return CubicCellManager();
    }
