...
OutputFile::OutputFile(const string &filename, int freq, int cacheFreq,
...
void OutputFile::flushCache() {
  if (myBuffer.str().size() > 0) {
    if (!reopen())
      THROWS(" Can not open \'" << myFilename << "\' for " << getId() << ".");
    myFile << myBuffer.str();
    reclose();
  }
  doFlushCache();
...
void OutputFile::getParameters(vector<Parameter> &parameter) const {
  ...
  parameter.push_back
    (Parameter(getId() + string("CloseTime"),
               Value(myCloseTime, ConstraintValueType::NotNegative()),
               1.0, Text("minimal time interval between two writes to close "
